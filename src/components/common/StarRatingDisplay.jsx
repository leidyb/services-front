// Ruta: src/components/common/StarRatingDisplay.jsx
import React from 'react';
import './StarRatingDisplay.css'; // Crearemos este CSS
...
const StarRatingDisplay = ({ rating, totalRatings, size = "1.2em" }) => {
    ...
    const halfStar = (rating % 1) >= 0.25 && (rating % 1) <= 0.75; // Umbral para media estrella
    const emptyStars = 5 - fullStars - (halfStar ? 1 : 0);
    const displayRating = rating.toFixed(1); // Mostrar con un decimal
    ...
    return (
        <div className="star-rating-display" style={{ fontSize: size }}>
            ...
            {totalRatings !== undefined && ( // Mostrar conteo si se provee
                <span className="star-rating-text">
                    ({displayRating} de {totalRatings} calif.)
                </span>
            )}
            {totalRatings === undefined && ( // Mostrar solo el promedio si no hay conteo
                 <span className="star-rating-text">
                    ({displayRating})
                </span>
            )}
